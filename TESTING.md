...
| Standalone first save | Submit one valid synthetic term from standalone Add Term.                             | A loadable `glossary.yaml` is created with that term, the form resets, the file uses `0600` permissions, and a support directory created by GlossaryGo uses `0700` permissions. Existing support-directory permissions stay unchanged. |
